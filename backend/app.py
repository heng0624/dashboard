...
from process_data import process_single_file_and_merge
import pandas as pd
import os
app = Flask(
    __name__,
...
def apply_filters(df):
    ...
    if "DATE PAID" in df.columns:
        ...
        if day and day != "ALL":
            ...
            last_day_of_month = (df["DATE PAID"] + pd.offsets.MonthEnd(0)).dt.day
            
            # Filter: exact day match OR (selected day > month's last day AND date is on that month's last day)
            mask = (df["DATE PAID"].dt.day == day_int) | ((day_int > last_day_of_month) & (df["DATE PAID"].dt.day == last_day_of_month))
            df = df[mask]
    ...
    # PL
    # =========================
    if pl and pl != "ALL" and "PL_1" in df.columns:
        df["PL_1"] = df["PL_1"].astype(str).str.upper().str.strip()
        df = df[df["PL_1"] == pl]

    # =========================
    # PLC
    # =========================
    if plc and plc != "ALL" and "SPL" in df.columns:
        df["SPL"] = df["SPL"].astype(str).str.upper().str.strip()
        df = df[df["SPL"] == plc]
...
def monthly_collection():
    ...
    # GROUP
    result = (
        df.groupby("MONTH", as_index=False)["RM PAID"]
        .sum()
        .sort_values("MONTH")
    )
...
def total_files():
    ...
    # =========================
    # COUNT UNIQUE ACCOUNTS (NOT ROWS)
    # =========================
    total_unique = df["ACCOUNT NO"].nunique() if "ACCOUNT NO" in df.columns else len(df)
    current_unique = current_df["ACCOUNT NO"].nunique() if "ACCOUNT NO" in current_df.columns else len(current_df)
    last_unique = last_df["ACCOUNT NO"].nunique() if "ACCOUNT NO" in last_df.columns else len(last_df)

    return jsonify({
        "total_files": int(total_unique),
...
def status_summary():
    ...
    # =========================
    # FIX DATE ISSUE (ASSIGN DATE_1 is priority)
    # =========================
    ...
    # =========================
    # BUILD RESULT (COUNT UNIQUE ACCOUNTS PER STATUS - CURRENT MONTH ONLY)
    # =========================
    result = {}
    ...
    for status in ["CTC", "UCT", "WIP", "ABORT"]:
        ...
        # Count unique accounts, not rows
        unique_count = status_df["ACCOUNT NO"].nunique() if "ACCOUNT NO" in status_df.columns else len(status_df)

        result[status] = {
            "total": int(unique_count),
...
def all_status():
    ...
    # GROUP BY MONTH, STATUS, SUB-STATUS AND COUNT UNIQUE ACCOUNTS
    result = (
        df.groupby(["MONTH", "REPORT STATUS", "SUB-STATUS"])["ACCOUNT NO"]
        .nunique()
...
def product_performance():
    ...
    result["ACHIEVEMENT"] = result.apply(
        lambda x: min((x["COLLECTION"] / x["TARGET"] * 100), 100) if x["TARGET"] > 0 else 0,
        axis=1
    )
...
def upload_data():
    ...
    if not file.filename.lower().endswith(('.xlsx', '.xlsm')):
        return jsonify({'error': 'Invalid file type. Only .xlsx and .xlsm allowed'}), 400
    ...
    try:
        ...
        # Invalidate caches
        import data_loader
        data_loader.DATA_CACHE = None
        data_loader.MONTHLY_CACHE = None
        data_loader.TARGET_CACHE = None
        ...
        # Remove temp file (handle permission errors gracefully)
        if os.path.exists(temp_path):
            try:
                os.remove(temp_path)
            except PermissionError:
                # File is still in use, that's ok - it will be there for next upload
                pass
